"""src/streaming/kafka_consumer_teja.py - Kafka consumer (Teja's version).

Reads sales messages from a Kafka topic and writes consumed records to CSV.
...
OBS:
  Copied from kafka_consumer_case.py and renamed.
  Phase 4/5 change: this version counts consumed messages by their Kafka
  message key (the region_id set by the producer) and logs the per-key
  breakdown in the summary. Output is written to consumed_sales_teja.csv.
  Observations: the producer and consumer never call each other directly -
  the producer writes to a Kafka topic and the consumer reads from it on its
  own schedule. Kafka keeps each message in the topic, so a consumer can
  start late and still read from the beginning. Watching the per-key counts
  shows how Kafka uses the message key to group related records (here,
...
def consume_messages(consumer: Any) -> tuple[int, dict[str, int]]:
    ...
    Returns:
        A tuple of the consumed message count and a dictionary counting
        consumed messages grouped by their Kafka message key.
    """
    ...
    while consumed_count < MAX_MESSAGES:
        ...
        # PHASE 4/5 CHANGE: tally each consumed message by its Kafka key.
        kafka_key = str(row.get("_kafka_key", "(none)"))
        key_counts[kafka_key] = key_counts.get(kafka_key, 0) + 1

        consumed_count += 1
        LOG.info("MESSAGE CONSUMED")
        LOG.info(f"consumed={consumed_count} (key={kafka_key})")

    return consumed_count, key_counts
...
def log_summary(
    ...
    """Log final summary statistics, including a count of messages by Kafka key."""
    LOG.info("Summary:")
    LOG.info(f"Consumed {consumed_count} message(s) from topic {settings.topic!r}.")
    LOG.info("Messages consumed by Kafka key:")
    for kafka_key, count in sorted(key_counts.items()):
        LOG.info(f"  {kafka_key} = {count}")
    log_path(LOG, "OUTPUT_CSV", OUTPUT_CSV)
    LOG.info("========================")
...
def main() -> None:
    ...
    try:
        consumed_count, key_counts = consume_messages(consumer)
    finally:
        consumer.close()
    ...
    log_summary(consumed_count, key_counts, settings)
